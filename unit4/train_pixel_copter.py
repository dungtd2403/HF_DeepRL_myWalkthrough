...
# # Create the env
# env = gym.vector.make(env_id, num_envs=3)
...
class Policy(nn.Module):
    ...
    def act(self, state):
        ...
        action = m.sample()
        # print(action)
        # print('action and log_prob',action.item(), m.log_prob(action))
        return action.item(), m.log_prob(action)

def reinforce(env,policy, optimizer, n_training_episodes, max_t, gamma, print_every,save_every):
    ...
    for i_episode in range(1, n_training_episodes+1):
        ...
        if i_episode % save_every ==0:
            ...
            torch.save(best_policy, f'./save_run/full_model_pixelcopter{i_episode}.pt')
...
if __name__ == '__main__':
    ...
    # Create the evaluation env
    eval_env = gym.vector.make(env_id, num_envs=3)
    ...
    score,cartpole_policy = reinforce(env,cartpole_policy,
                   cartpole_optimizer,
